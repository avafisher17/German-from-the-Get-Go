...
document.addEventListener("DOMContentLoaded", () => {
    ...
    function goToSubpage(index) {
        if (index < 0 || index >= subpages.length) {
            return;
        }
    ...
    back.addEventListener("click", () => {
        if (currentIndex === 0) {
            ...
            let userConfirm = confirm("Are you sure? Going back will reset your game progress!");
            if (userConfirm) {
                goToSubpage(currentIndex - 1);
                resetGame();
                nextDisabled = false;
            }
            else {
    ...
    nextQuestionButton.addEventListener("click", () => {
        if (correctRevealed) {
            if (currentIndex === 3) {
                if (usedImages.length === questionPool.length * 2) { //Allows each word to be practiced twice
                    subpages[currentIndex].classList.add("hidden");
                    currentIndex++;
                    subpages[currentIndex].classList.remove("hidden");
